...
class EventoRouteTest {
    ...
    @Test
    @Timeout(10)
    fun `deve criar um evento e retornar corretamente`() = testApplication {
        application { configureRouting() }
        val json = Json { ignoreUnknownKeys = true }
        ...
        // Faz login com o organizador para obter o token
        val loginRequest1 = LoginRequest("org3@usp.br", "orgpass")
        val loginResponse1 = client.post("/api/users/login") {
            contentType(ContentType.Application.Json)
            setBody(json.encodeToString(loginRequest1))
    ...
    @Test
    @Timeout(10)
    fun `deve criar um evento e adicionar imagens corretamente`() = testApplication {
        application { configureRouting() }
        val json = Json { ignoreUnknownKeys = true }
        ...
        val registerRequestOrg = RegisterRequest(
            email = "org4@usp.br",
            username = "Organizador que cria e bota imagem",
            password = "orgpass",
            accountType = "organizador",
            profilePhoto = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSABqV-VQBSF1Qyj1Qyo6RiLfpA0McI1leTvQ&s"
        )
        ...
        val eventoRequest = EventoRequest(
            titulo = "JunIME",
            descricao = "Festa junina do IME",
            dataHora = LocalDateTime.now().plusDays(7).toString(),
            localizacao = "Estacionamento do bloco B do IME",
            categoria = "Festa",
            organizadorId = organizadorId
        )
        ...
        assertEquals(HttpStatusCode.Created, eventoResponse.status)
        val createdEvento = json.decodeFromString<Evento>(eventoResponse.bodyAsText())
        val eventoId = createdEvento.id!!
        ...
        assertEquals(HttpStatusCode.Created, imagemResponse.status)

        // Validações
        val eventRepo = EventoRepository()
        val eventoAtualizado = eventRepo.findById(eventoId)
        assertNotNull(eventoAtualizado)
        assertEquals(1, eventoAtualizado.imagens.size)
        assertEquals("https://link.com/img.jpg", eventoAtualizado.imagens[0].url)
    }
